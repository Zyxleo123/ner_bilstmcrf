...
我将所有不可能的转换的分数设置为`-10000.`，开始训练，但是可能是由于数值太大，导致不能收敛；于是改成`-5.`。做了这个改动以后，模型在validation set和test set上的F1没有进一步上升，但是最低时的train loss降低了，这个是因为partition中不可能的路径的得分更低了。也就是说CRF可以自己学到这些，虽然看上去没有学到。
